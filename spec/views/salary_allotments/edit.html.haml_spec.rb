...
describe "salary_allotments/edit" do
  ...
  it "render form contains salary heads for selected employee" do
    ...
    employee_details = FactoryGirl.create(:employee_detail, :employee_id => employee.id, :attendance_configuration_id=>attendance_configuration.id,:branch_id=>branch.id, :financial_institution_id=>financial_institution.id)
    salary_head = FactoryGirl.create(:salary_head)
    salary_group_detail = FactoryGirl.create(:salary_group_detail,:salary_head_id=>salary_head.id,:calc_type=>"Every Month")
